...
// Õâ²¿·ÖÓ¦¸ÃÊÇÊôÓÚILedService.cppµÄÄÚÈÝ  -- ¿ªÊ¼
//
/*
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        ...
        case SET_DEVICE_NAME: {
            ...
            setDeviceName(const_cast<char *>(name));  //ÓÉLedServiceÊµÏÖ
            return NO_ERROR;
        } break;
        //Ìí¼ÓsetOnµÄ·þÎñ¶ËÍ¨ÐÅ,ÓÉLedServiceÊµÏÖ
        case LED_ON: {
            CHECK_INTERFACE(ILedService, data, reply);
            const char *name = data.readCString();
            setOn(const_cast<char *>(name));
            return NO_ERROR;
        } break;
        default:
...
// Õâ²¿·ÖÓ¦¸ÃÊÇÊôÓÚILedService.cppµÄÄÚÈÝ   -- ½áÊø
//
